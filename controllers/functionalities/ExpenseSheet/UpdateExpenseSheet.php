...
if (isset($_GET['updateid'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $result = get_receipts_data();
        $receipts_id = $result['receipts_id'];
    
        $expense_sheet = [':ui' => $_SESSION['id'], ':ri' => $receipts_id, ':rd' => $_POST['request_date'], ':sd' => $_POST['start_date'], ':ed' => $_POST['end_date']];
        $receipts = [];
            ...
            if ($expense_sheet[':tc'] != 4) {
                if (!empty($_POST['transport_expense'])) {
                    ...
                    if (!empty($_FILES['transport_expense_file']['name'])) {
                        $receipts[':tef'] = $target_dir . 'transport/' . $receipts_id . '_' . basename($_FILES['transport_expense_file']['name']);
                    } else {
        ...
        if (!empty($_POST['accommodation_expense'])) {
            ...
            if (!empty($_FILES['accommodation_expense_file']['name'])) {
                $receipts[':aef'] = $target_dir . 'accommodation/' . $receipts_id . '_' . basename($_FILES['accommodation_expense_file']['name']);
            } else {
        ...
        if (!empty($_POST['food_expense'])) {
            ...
            if (!empty($_FILES['food_expense_file']['name'])) {
                $receipts[':fef'] = $target_dir . 'food/' . $receipts_id . '_' . basename($_FILES['food_expense_file']['name']);
            } else {
        ...
        if (!empty($_POST['other_expense'])) {
            ...
            if (!empty($_FILES['other_expense_file']['name'])) {
                $receipts[':oef'] = $target_dir . 'other/' . $receipts_id . '_' . basename($_FILES['other_expense_file']['name']);
            } else {
